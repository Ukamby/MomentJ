...
public class FormatTest {
    ...
    public void formatYY() {
        ...
        Moment b = Moment.moment(dateWithMilliseconds(2009, 1, 14, 15, 25, 50, 125));
    ...
    public void formatescapebrackets() {
        ...
        var b = Moment.moment(new Date(2009, 1, 14, 15, 25, 50, 125));
    ...
    public void formatmilliseconds() {
        test.expect(6);
        var b = Moment.moment(new Date(2009, 1, 14, 15, 25, 50, 123));
    ...
    public void formattimezone() {
        ...
        var b = Moment.moment(new Date(2010, 1, 14, 15, 25, 50, 125));
        var explanation = "Moment.moment().format("z") = " + b.format("z") + " It should be something like "PST""
        if (Moment.moment().zone() === -60) {
    ...
    public void formatmultiplewithzone() {
        ...
        var b = Moment.moment("2012-10-08 -1200", ["YYYY ZZ", "YYYY-MM-DD ZZ"]);
    ...
    public void isDST() {
        ...
        var janOffset = new Date(2011, 0, 1).getTimezoneOffset(),
            ...
            jan1 = Moment.moment([2011]),
            jul1 = Moment.moment([2011, 6]);
    ...
    public void unixtimestamp() {
        ...
        var m = Moment.moment("1234567890.123", "X");
    ...
    public void zone() {
        ...
        if (Moment.moment().zone() > 0) {
            test.ok(Moment.moment().format("ZZ").indexOf("-") > -1, "When the zone() offset is greater than 0, the ISO offset should be less than zero");
        }
        if (Moment.moment().zone() < 0) {
            test.ok(Moment.moment().format("ZZ").indexOf("+") > -1, "When the zone() offset is less than 0, the ISO offset should be greater than zero");
        }
        if (Moment.moment().zone() == 0) {
            test.ok(Moment.moment().format("ZZ").indexOf("+") > -1, "When the zone() offset is equal to 0, the ISO offset should be positive zero");
        }
        if (Moment.moment().zone() === 0) {
           test.equal(Moment.moment().zone(), 0, "moment.fn.zone should be a multiple of 15 (was " + Moment.moment().zone() + ")");
        } else {
           test.equal(Moment.moment().zone() % 15, 0, "moment.fn.zone should be a multiple of 15 (was " + Moment.moment().zone() + ")");
        }
        test.equal(Moment.moment().zone(), new Date().getTimezoneOffset(), "zone should equal getTimezoneOffset");
        test.done();
    }
    ...
    public void defaultformat() {
        ...
        test.ok(isoRegex.exec(Moment.moment().format()), "default format (" + Moment.moment().format() + ") should match ISO");
        test.done();
    }
    ...
    public void toJSON() {
        var supportsJson = typeof JSON !== "undefined" && JSON.stringify && JSON.stringify.call,
            date = Moment.moment("2012-10-09T21:30:40.678+0100");
    ...
    public void weeksformat() {
        ...
        for (var i in cases) {
            ...
            var the = Moment.moment(i).format("WW");
    ...
    public void isoweekdayformats() {
        ...
        test.equal(Moment.moment([1985, 1,  4]).format("E"), "0", "Feb  4 1985 is Monday    -- 0th day");
        test.equal(Moment.moment([2029, 8, 18]).format("E"), "1", "Sep 18 2029 is Tuesday   -- 1st day");
        test.equal(Moment.moment([2013, 3, 24]).format("E"), "2", "Apr 24 2013 is Wednesday -- 2nd day");
        test.equal(Moment.moment([2015, 2,  5]).format("E"), "3", "Mar  5 2015 is Thursday  -- 3nd day");
        test.equal(Moment.moment([1970, 0,  2]).format("E"), "4", "Jan  2 1970 is Friday    -- 4th day");
        test.equal(Moment.moment([2001, 4, 12]).format("E"), "5", "May 12 2001 is Saturday  -- 5th day");
        test.equal(Moment.moment([2000, 0,  2]).format("E"), "6", "Jan  2 2000 is Sunday    -- 6th day");
    ...
    public void weekdayformats() {
        ...
        test.equal(Moment.moment([1985, 1,  6]).format("e"), "0", "Feb  6 1985 is Wednesday -- 0th day");
        test.equal(Moment.moment([2029, 8, 20]).format("e"), "1", "Sep 20 2029 is Thursday  -- 1st day");
        test.equal(Moment.moment([2013, 3, 26]).format("e"), "2", "Apr 26 2013 is Friday    -- 2nd day");
        test.equal(Moment.moment([2015, 2,  7]).format("e"), "3", "Mar  7 2015 is Saturday  -- 3nd day");
        test.equal(Moment.moment([1970, 0,  4]).format("e"), "4", "Jan  4 1970 is Sunday    -- 4th day");
        test.equal(Moment.moment([2001, 4, 14]).format("e"), "5", "May 14 2001 is Monday    -- 5th day");
        test.equal(Moment.moment([2000, 0,  4]).format("e"), "6", "Jan  4 2000 is Tuesday   -- 6th day");
    ...
    public void toStringisjusthumanreadableformat() {
        ...
        var b = Moment.moment(new Date(2009, 1, 5, 15, 25, 50, 125));
